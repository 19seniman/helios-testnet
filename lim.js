...
function getProviderWithProxy(proxyUrl, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // The above line passes `fetch` and `agent` correctly for ethers v6
...
async function stake(wallet, amount, validatorAddress, validatorName) {
  try {
    ...
    try {
      await makeJsonRpcCall("eth_getAccountLastTransactionsInfo", [wallet.address]);
    } catch (rpcError) {
      addLog(`Failed to sync with portal via JSON-RPC: ${rpcError.message}`, "error");
    }
...
function safeRender() {
  ...
  setTimeout(() => {
    try {
      if (!isHeaderRendered) {
        figlet.text("SHADOW ECONOMY", { font: "Lobster" }, (err, data) => {
